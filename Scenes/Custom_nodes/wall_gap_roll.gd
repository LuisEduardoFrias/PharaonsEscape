...
func _ready() -> void:
	if is_block:
		Global.check_door(LevelsData.Levels.LEVEL3, name, self)


func update() -> void:
	var reg :=  block_sprite if is_block else unblock_sprite
	tml.set_cell(Vector2i(0, 0), 2, reg)
	tml.set_cell(Vector2i(0, -1), 2, reg, TileSetAtlasSource.TRANSFORM_FLIP_V )
